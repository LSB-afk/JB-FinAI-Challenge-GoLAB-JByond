...
function rmoBuildIntegratedDeliverable(caseRow, agentDeliverables) {
  ...
  const body = [
    ...
    `- 상담 유형: ${(RMO_CASE_TYPES[caseRow.caseType] || {}).label || caseRow.caseType || "-"}`,
    `- 처리 목표: ${caseRow.goal || "담당 RM 검토"}`,
    `- 직원 액션: 산출물 근거 확인 → 필요 시 고객 정보 확인 → A 승인 또는 보완 지시`,
    ``,
    `## 2. 상황`,
    caseRow.situation,
    ``,
    ...
    `- high/critical 케이스는 자동 완료하지 않고 승인 라우팅을 통해 담당자 확인을 남깁니다.`,
    `- 고객 안내·상환유예·정책자금 안내는 실제 업무 규정과 고객 동의 절차를 거친 뒤 진행합니다.`,
    ``,
    `## 7. 사용 출처`,
