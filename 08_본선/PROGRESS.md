...
- [~] 계열사=전북은행+JB우리캐피탈 · 구동=하이브리드 확정 / **D2 실동작 도메인 택1**·DB/실연동 범위 최종 대기
...
- [~] **스코프 상태**: 계열사=전북은행+JB우리캐피탈 / 구동=하이브리드 실동작(paperclip式) / 도메인=여신 더미 + 전세사기·보이스피싱 중 실동작 택1 / 연동=혼합.
- [ ] **다음**: D2 실동작 도메인 택1 → 앱 설계 문서 확정(`03_제품/04_tech`·`03_제품/03_ux`) → 발표자료 PDF·기능명세서 PDF(+변경이력)·GitHub README·시연영상.
