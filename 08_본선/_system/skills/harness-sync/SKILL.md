...
### 4. MOC/frontmatter 정합성 검증 **[스크립트]**

```bash
node 08_본선/_system/skills/canon-moc-sync/sync.mjs
```

미해결 위키링크 탐지 · 중복 파일명 차단 · frontmatter 필수 필드(`tags`, `date`, `up`) 보정.
`_MOC_` 접두어·`_system/` 경로 파일은 MOC 자동 추가 대상에서 제외.
...
[harness-sync] 동기화 완료 — 2026-06-27
  ✓ 1. 프롬프트 로그 append (n건)
  ✓ 2. 텔레메트리 집계 (intake X행 → 4파일 갱신)
  ✓ 3. 플러그인 레지스트리 재생성 (활성 N, 비활성 M)
...
- **MOC 파일 건드리지 않음** — `_MOC_` 접두어·`_system/` 내 MOC는 `canon-moc-sync`가 자동 추가만 수행
...
- [[registry-plugins|플러그인 레지스트리]] — 재생성 대상
- [[registry-mcp|MCP 레지스트리]] — 재생성 대상
- [[telemetry-aggregator|텔레메트리 집계기]] — 2단계 호출
