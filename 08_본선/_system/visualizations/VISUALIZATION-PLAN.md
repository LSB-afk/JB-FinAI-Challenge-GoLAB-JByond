...
|---|---|---|---|---|---|
| `project-master-timeline.excalidraw` | 프로젝트 시작부터 본선까지의 요일 포함 마스터 타임라인 | 발표·심사자 | `git log`, [[기능-변경이력]], [[PROGRESS]] | 일정·마일스톤 변경 | 날짜·요일·본선 D-day·심사항목이 보임 |
...
| `update-control-tower.excalidraw` | 지금 어디까지 왔고 무엇이 막혔는지 추적 | 팀 운영 | [[PROGRESS]], [[PLAN]], `phase-ledger.csv` | PROGRESS 변경 | Done/In Progress/Waiting/Risk가 보임 |
| `demo-video-storyboard.excalidraw` | 시연영상 제작용 12컷 스토리보드 | 발표·영상 제작 | [[본선-시연-시나리오]], [[03-발표-시연-스크립트]] | 시연 흐름·영상 규칙 변경 | 컷·자막·화면요소·증빙요소가 있음 |
| `evidence-traceability-board.excalidraw` | 핵심 주장→근거→산출물→심사항목 연결 | 발표 백업 | `_canon`, D결과, 제출 문서, 심사기준 | 리서치·제출 문서 변경 | 주장마다 출처와 심사항목이 연결됨 |
...
| `research-to-product-funnel.excalidraw` | 딥리서치가 제품 결정으로 내려온 흐름 제시 | 제품·발표 | D1~D19, D+a/b, 갭감사, 제품정의 | 리서치 회수·제품 반영 변경 | 리서치 묶음과 제품결정이 이어짐 |
