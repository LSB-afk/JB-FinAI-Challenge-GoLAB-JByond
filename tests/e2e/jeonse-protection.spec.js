...
test("보드 SUB 실행 큐: 숫자/방향키/Space/Enter와 딤드 로딩, 산출물 생성", async ({ page }) => {
  await page.goto("/index.html#/roles/jeonse-protection/board");
  await expect(page.locator(".jpo-sub-workspace")).toContainText("SUB · 선택된 케이스의 실행 큐");
  await expect(page.locator(".jpo-command-strip")).toContainText("Space 다음");

  await page.keyboard.press("1");
  await expect(page.locator(".jpo-key-overlay")).toContainText("1 선택");
  await expect(page.locator(".jpo-card.is-selected")).toHaveCount(1);

  const focusedBefore = await page.locator(".jpo-node-card.is-focused").first().getAttribute("data-jpo-node");
  await page.keyboard.press("Space");
  await expect(page.locator(".jpo-key-overlay")).toContainText("Space 다음");
  const focusedAfter = await page.locator(".jpo-node-card.is-focused").first().getAttribute("data-jpo-node");
  expect(focusedAfter).not.toBe(focusedBefore);

  await page.keyboard.press("Enter");
  await expect(page.locator(".jpo-node-card.is-running .jpo-run-overlay")).toContainText("조금만 기다려주세요");
  await expect(page.locator(".jpo-node-card.is-running .jpo-run-overlay")).toContainText("40%");
  await page.waitForFunction((key) => JSON.parse(window.localStorage.getItem(key)).jeonse_deliverables.length > 3, JPO_DB_KEY);
  await expect(page.locator(".jpo-sub-workspace")).toContainText(".md", { timeout: 4_000 });

  const generated = await page.evaluate((key) => {
    const db = JSON.parse(window.localStorage.getItem(key));
    return {
      deliverables: db.jeonse_deliverables.length,
      audits: db.jeonse_audit_logs.filter((x) => x.action === "JPO_DELIVERABLE_CREATED").length,
    };
  }, JPO_DB_KEY);
  expect(generated.deliverables).toBeGreaterThan(3);
  expect(generated.audits).toBeGreaterThan(0);
});

test("오른쪽 컨텍스트 패널은 선택 항목 요약을 보여주고 드래그 리사이즈된다", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 920 });
  await page.goto("/index.html#/roles/jeonse-protection/board");
  await page.keyboard.press("1");
  await expect(page.locator("#context-panel")).toContainText("선택 케이스");
  await expect(page.locator("#context-panel")).toContainText("다음 액션");
  await expect(page.locator("#context-panel")).toContainText("실행 큐");
  await expect(page.locator("#context-panel")).not.toContainText("전용 하네스");

  const before = await page.locator(".properties-panel").boundingBox();
  const handle = await page.locator("#properties-resizer").boundingBox();
  ...
  await page.mouse.up();
  await page.waitForFunction((width) => document.querySelector(".properties-panel").getBoundingClientRect().width > width + 80, before.width);
  const after = await page.locator(".properties-panel").boundingBox();
  expect(after.width).toBeGreaterThan(before.width + 80);
});

test("업무 기능/에이전트 저장소 카드가 도메인·상태·위험도·입출력·검토 정보를 노출한다", async ({ page }) => {
  await page.goto("/index.html#/roles/jeonse-protection/capability-repository");
  const firstCard = page.locator(".jpo-cap-card").first();
  await expect(firstCard).toContainText("도메인");
  await expect(firstCard).toContainText("위험도");
  await expect(firstCard).toContainText("입력 → 출력");
  await expect(firstCard).toContainText("사람 검토");
  await expect(firstCard).toContainText("연결 에이전트");
  await firstCard.click();
  await expect(page.locator("#context-panel")).toContainText("기능명");
  await expect(page.locator("#context-panel")).toContainText("입력값");
  await expect(page.locator("#context-panel")).toContainText("출력값");
});
...
test("경·공매 접수: 스냅샷·위험 신호·체크·감사·승인 기록과 high 이상 human review", async ({ page }) => {
  ...
  await page.locator('input[name="evidenceFiles"]').setInputFiles({
    name: "auction-notice-summary.txt",
    mimeType: "text/plain",
    buffer: Buffer.from("경매 기일 D-9 요약. 테스트 파일 원문은 저장하지 않음."),
  });
  ...
  const created = await page.evaluate(([key, id]) => {
    ...
    return {
      ...
      files: db.jeonse_evidence_files.filter((x) => x.caseId === item.id).map((x) => x.fileName),
      fileAudit: db.jeonse_audit_logs.some((x) => x.action === "JPO_FILE_METADATA_CAPTURED" && x.caseId === item.id),
    };
  ...
  expect(created.files).toContain("auction-notice-summary.txt");
  expect(created.fileAudit).toBe(true);
